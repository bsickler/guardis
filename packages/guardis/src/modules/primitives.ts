...
/**
 * Wraps a TypeGuard<Date> with chainable comparison methods (gt, gte, lt, lte).
 * Each method delegates to .extend() and wraps the result for further chaining.
 */
function withDateComparisons(guard: TypeGuard<Date>): TypeGuard<Date> & DateTypeGuard {
  const dated = guard as TypeGuard<Date> & DateTypeGuard;
  dated.gt = (threshold) =>
    withDateComparisons(
 ...
 * ```
 */
export const isDate: TypeGuard<Date> & DateTypeGuard = withDateComparisons(
  createTypeGuard("Date", (t) => t instanceof Date ? t : null),
);
...
export { isEmpty, isIterable, isNil };
