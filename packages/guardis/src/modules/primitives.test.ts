...
import { createTypeGuard, isInstanceOf } from "../guard.ts";
...
Deno.test("date comparison methods", async (t) => {
  await t.step("gt", () => {
    const threshold = new Date("2023-01-01");
    assert(isDate.gt(threshold)(new Date("2023-01-02")));
    assertFalse(isDate.gt(threshold)(new Date("2023-01-01")));
    assertFalse(isDate.gt(threshold)(new Date("2022-12-31")));
    assertFalse(isDate.gt(threshold)("2023-01-02"));
  });

  await t.step("lt", () => {
    const threshold = new Date("2023-12-31");
    assert(isDate.lt(threshold)(new Date("2023-12-30")));
    assertFalse(isDate.lt(threshold)(new Date("2023-12-31")));
    assertFalse(isDate.lt(threshold)(new Date("2024-01-01")));
    assertFalse(isDate.lt(threshold)("2023-12-30"));
  });

  await t.step("chaining gt and lt", () => {
    const isIn2023 = isDate.gt(new Date("2022-12-31")).lt(new Date("2024-01-01"));
    assert(isIn2023(new Date("2023-06-15")));
    assertFalse(isIn2023(new Date("2022-12-31")));
    assertFalse(isIn2023(new Date("2024-01-01")));
  });

  await t.step("gte", () => {
    const threshold = new Date("2023-01-01");
    assert(isDate.gte(threshold)(new Date("2023-01-01")));
    assert(isDate.gte(threshold)(new Date("2023-06-01")));
    assertFalse(isDate.gte(threshold)(new Date("2022-12-31")));
    assertFalse(isDate.gte(threshold)("2023-06-01"));
  });

  await t.step("lte", () => {
    const threshold = new Date("2023-12-31");
    assert(isDate.lte(threshold)(new Date("2023-12-31")));
    assert(isDate.lte(threshold)(new Date("2023-06-01")));
    assertFalse(isDate.lte(threshold)(new Date("2024-01-01")));
    assertFalse(isDate.lte(threshold)("2023-06-01"));
  });

  await t.step("chaining gte and lte for a range", () => {
    const isIn2023 = isDate.gte(new Date("2023-01-01")).lte(new Date("2023-12-31"));
    assert(isIn2023(new Date("2023-06-15")));
    assert(isIn2023(new Date("2023-01-01")));
    assert(isIn2023(new Date("2023-12-31")));
    assertFalse(isIn2023(new Date("2022-12-31")));
    assertFalse(isIn2023(new Date("2024-01-01")));
  });
  ...
  await t.step("validate method", () => {
    ...
    assertEquals(isDate.gte(threshold).validate(date), { value: date });
    assertEquals(isDate.gte(threshold).validate(new Date("2022-01-01")), {
      issues: [{
        message: `Expected >= ${threshold.toISOString()}. Received: "2022-01-01T00:00:00.000Z"`,
      }],
    });
  });
  ...
  await t.step("strict method", () => {
    const threshold = new Date("2023-01-01");
    isDate.gte(threshold).strict(new Date("2023-06-01"));
    assertThrows(() => isDate.gte(threshold).strict(new Date("2022-01-01")));
  });

  await t.step("or after comparison", () => {
    const guard = isDate.gte(new Date("2023-01-01")).or(isNull);
    assert(guard(new Date("2023-06-01")));
    assert(guard(null));
    assertFalse(guard(new Date("2022-01-01")));
  });
});
